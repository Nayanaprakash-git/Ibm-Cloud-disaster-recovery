# Ibm-Cloud-disaster-recoveryLet me break down the project details, setup instructions, and the .env file configuration for you based on the provided code. This is a Database Backup & Restore Dashboard built with Node.js, Express, IBM Cloud Object Storage (COS), PostgreSQL, WebSockets, and a simple EJS frontend. It allows manual and automated backups, restores, disaster recovery, and log monitoring.
...
PostgreSQL (with pg_dump and psql CLI tools installed)
IBM Cloud Object Storage account (for storing backups)
...
Access: Open http://localhost:3000 and log in with the ADMIN_USER and ADMIN_PASS from .env.
Manual Backup: Click "Create New Backup" to generate and upload a backup.
Restore: Select a backup and click "Restore" to overwrite the current database.
Disaster Recovery: Click "Disaster Recovery" to restore a backup into a temporary database.
Download/Delete: Use the respective buttons to download or delete backups.
Logs: View recent logs and filter them using the search bar.
Auto-Backup: If scheduled in .env, backups run automatically.
Troubleshooting
